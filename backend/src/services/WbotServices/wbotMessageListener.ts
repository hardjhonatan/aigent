...
const handleMessageIntegration = async (
  ...
  console.log(queueIntegration.type);
  if (queueIntegration.type === "n8n") {
    if (queueIntegration?.urlN8N) {
      ...
      try {
        ...
        if (response && response.body) {
          let webhookResponse = response.body;

          // Verificar se a resposta é um array e pegar o primeiro elemento
          if (Array.isArray(webhookResponse) && webhookResponse.length > 0) {
            webhookResponse = webhookResponse[0];
          }

          // Extrair o conteúdo de 'output'
          const responseText = webhookResponse.output || 'Não foi possível processar a resposta.';

          console.log(responseText);
          console.log('teste', webhookResponse);
          ...
          // Se houver mídia na resposta, enviar também
          if (webhookResponse.mediaUrl) {
            const media = {
              image: { url: webhookResponse.mediaUrl }
            };
            await wbot.sendMessage(
              `${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`,
              media
          ...
          // Se necessário, atualizar o status do ticket
          if (webhookResponse.closeTicket) {
            await UpdateTicketService({
  ...
  } else if (queueIntegration.type === "dify") {
    const apiUrl = "https://api.dify.ai/v1/chat-messages";
    const apiKey = queueIntegration.urlN8N;
    const userIdentifier = ticket.id.toString(); // Use ticket.id as the identifier

    let contentType: string;
    let messageBody: any = {
      user: userIdentifier,
      auto_generate_name: true,
      response_mode: "blocking", // Change to "blocking" if you want synchronous responses,
      inputs: {}
    };

    // Determine the type of message and set the content
    if (msg.message?.conversation || msg.message?.extendedTextMessage?.text) {
      messageBody.query = msg.message?.conversation || msg.message?.extendedTextMessage?.text;
      console.log(messageBody.query);
      contentType = "application/json";
    } else if (msg.message?.imageMessage) {
      const mediaUrl = msg.message.imageMessage?.url; // Ensure your download logic sets this
      messageBody.query = "Image received, processing...";
      messageBody.files = [
        {
          type: "image",
      ...
      const mediaUrl = msg.message.audioMessage?.url; // Ensure your download logic sets this
      // For audio messages, Dify might need a different processing
      const audioProcessingResponse = await processAudioMessageWithDify(apiUrl, apiKey, mediaUrl, userIdentifier);
      return sendWhatsappMessage(wbot, ticket, audioProcessingResponse);
    } else {
    ...
    try {
      ...
      if (contentTypeHeader && contentTypeHeader.indexOf("application/json") !== -1) {
        ...
        if (result && result.answer) {
          ...
          console.log("Resposta completa do Dify:", result);
        } else {
          console.log("Resposta inesperada do Dify:", result);
          throw new Error("Resposta inesperada do Dify");
        }
      } else {
      ...
      console.error("Error processing message with Dify:", error);
    }
  } else if (queueIntegration.type === "flowise") {
    if (queueIntegration?.urlN8N) {
      // Extrair a mensagem atual do usuário
      ...
      if (hasMedia) {
        try {
          ...
          // Preparar os dados da mídia para envio
          const mediaData = {
            data: `data:${media.mimetype};base64,${media.data.toString('base64')}`,
            type: 'file',
            name: media.filename,
            mime: media.mimetype
          };

          // Adicionar dados da mídia ao objeto de requisição
          data.uploads = [mediaData];
        } catch (error) {
      ...
      try {
        const response = await fetch(queueIntegration.urlN8N, {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
          },
        ...
        if (contentType && contentType.indexOf("application/json") !== -1) {
          ...
          if (result && result.text) {
            ...
            console.log("Resposta completa do Flowise:", result);
          } else {
            console.log("Resposta inesperada do Flowise:", result);
            throw new Error("Resposta inesperada do Flowise");
          }
        } else {
          // Se não for JSON, lê como texto
          const text = await response.text();
  ...
  } else if (queueIntegration.type === "openaiassist") {
    console.log("ASSIST INICIO");
    const apiKey = queueIntegration.typebotSlug;
    const assistantId = queueIntegration.urlN8N;
    const userMessage = msg.message?.conversation || msg.message?.extendedTextMessage?.text || 'analise';

    console.log("ASSIST 2", apiKey, assistantId, userMessage);

    let threadId = await getThreadIdForTicket(ticket);

    // Se não existir threadId, cria um novo
    if (!threadId) {
      ...
      console.log("ASSIST threadResponse", threadResponse);
      ...
      await saveThreadIdForTicket(ticket, threadId);
    }

    console.log("ASSIST threadId", threadId);

    // Adicionar mensagem ao thread existente
    const messageBody = {
      role: 'user',
      content: userMessage
    };
    console.log("ASSIST messageBody", messageBody);
    ...
    console.log("ASSIST messageResponse", messageResponse);
    ...
    console.log("ASSIST runResult", runResult);
    const runId = runResult.id;

    let runCompleted = false;
    while (!runCompleted) {
      await new Promise(resolve => setTimeout(resolve, 5000)); // Espera 5 segundos antes de verificar novamente
      ...
      console.log("ASSIST statusResult", statusResult);

      if (statusResult.status === "completed") {
        runCompleted = true;

        // Buscar e exibir a mensagem de resposta do assistente
        ...
        console.log("ASSIST statusResult", messagesResponse);
        ...
        const responseText = assistantMessage ? assistantMessage.content : "No response from assistant";
        ...
        console.log("Resposta completa do assistente OpenAI:", responseText);
      } else if (statusResult.status === "failed") {
        throw new Error("Falha na execução do assistente OpenAI");
      }
    }
...
const getThreadIdForTicket = async (ticket: Ticket): Promise<string | null> => {
  return ticket.promptId;
};

const saveThreadIdForTicket = async (ticket: Ticket, threadId: string): Promise<void> => {
  await ticket.update({ promptId: threadId });
};
...
const handleMessage = async (
  ...
  console.log("Received message:", {
    ...
    fromMe: msg.key.fromMe,
    hasMedia: msg.message?.audioMessage || msg.message?.imageMessage || msg.message?.videoMessage || msg.message?.documentMessage || msg.message?.documentWithCaptionMessage
  });
  ...
  try {
    ...
    console.log("Last message found:", lastMessage, unreadMessages, whatsapp.complationMessage, contact);
    if (unreadMessages === 0 && whatsapp.complationMessage && formatBody(whatsapp.complationMessage, contact).trim().toLowerCase() === lastMessage?.body.trim().toLowerCase()) {
      console.log("Returning early due to completion message match");
      return;
    }
    ...
    console.log("verificando contagem de queue");
    console.log(ticket.queue);
    ...
    if (whatsapp.queues.length > 1 && ticket.queue) {
      console.log("mais de um queue2");
      if (ticket.chatbot && !msg.key.fromMe) {
        console.log("queue === 2b");
        console.log("Before handleChartbot:", {
          id: ticket.id,
